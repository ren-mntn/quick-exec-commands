...
const App: React.FC = () => {
      ...
      switch (message.type) {
        ...
        case 'commandAdded':
          if (message.payload.success) {
            setMessage({ type: 'success', text: 'コマンドが正常に追加されました' });
            setShowForm(false);
          } else {
            setMessage({ type: 'error', text: message.payload.error || 'コマンドの追加に失敗しました' });
          }
          break;
      }
    };
  ...
  const handleAddCommand = (command: Omit<QuickCommand, 'id' | 'createdAt' | 'updatedAt'>) => {
    vscode.postMessage({
      type: 'addCommand',
      payload: command
    });
  ...
  const groupedCommands = React.useMemo(() => {
    const grouped: { [key: string]: QuickCommand[] } = {
      ...
      global: commands.filter(cmd => cmd.category === 'global' && !cmd.isFavorite),
      repository: commands.filter(cmd => cmd.category === 'repository' && !cmd.isFavorite)
    };
